...
$queryf=mysqli_query($link, "SELECT * FROM reports");
$row=mysqli_fetch_assoc($queryf);
$status=$row['status'];

// SELECT notifications.notificationID,notifications.numPlate,notifications.description,
//notifications.phone,notifications.status,reports.reportID FROM notifications 
//LEFT OUTER JOIN reports ON notifications.notificationID = reports.notificationID
